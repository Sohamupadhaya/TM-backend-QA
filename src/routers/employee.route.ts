...
import {
  ...
  loginEmployeeInAdmin,
  getEmployeeProfile,
  uploadProfilePicture,
  getEmployeeByID,
  getEmployeeDetails,
  sendDeletedEmpReport,
  forgetPassword,
  verifyOtp,
  resetPassword
...
router.get(
  ...
  catchAsync(getEmployeeProfile)
);
router.patch(
  "/upload/:employeeId",
  catchAsync(isAuthenticatedEmployee),
  userPhoto.single("profilePicture"),
...
router.get(
  "/employe-by-Id/:employeeId",
  isAuthenticated,
  checkPermission("get", "employee"),
  catchAsync(getEmployeeByID)
);
router.get(
  "/employee-details/:employeeId",
  isAuthenticated,
  checkPermission("get", "employee"),
  catchAsync(getEmployeeDetails)
);
router.get(
  "/deleted-emp-report",
  isAuthenticated,
  catchAsync(sendDeletedEmpReport)
);
