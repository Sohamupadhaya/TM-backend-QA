...
export const addRole = async (
  ...
  if (!roleName || !description) {
    return next(
      customErrorHandler(res, "Please provide the role and description.", 401)
    );
  }
  ...
  return res.json({
    message: `${roleName} successfully added.`,
    roles: createNewRole,
  });
};
...
export const getAllRoleOfCompany = async (
  ...
  return res.json({ message: "All roles of the company", Data: allRoles });
};
...
export const assignRoleToEmployee = async (
  ...
  if (!roleId || !employeeId) {
    return next(
      customErrorHandler(res, "Please provide all the information", 400)
    );
  }
  const company = await prisma.company.findFirst({
    where: {
